...
import { ArrowRight, ChevronLeft, Clock3, FlaskConical } from "lucide-react";
import type { MenuItem } from "@/types/domain";
import { money } from "@/lib/mock-data";

export function SectionHeading({ eyebrow, title, href }: { eyebrow?: string; title: string; href?: string }) {
  return <div className="mb-4 flex items-end justify-between px-5"><div>{eyebrow && <p className="mb-1 text-xs font-extrabold uppercase tracking-[.16em] text-coral">{eyebrow}</p>}<h2 className="text-[1.45rem] font-black tracking-tight text-ink">{title}</h2></div>{href && <Link className="-my-3 py-3 text-sm font-bold text-coral" href={href}>See all</Link>}</div>;
}

export function FoodCard({ item, compact = false }: { item: MenuItem; compact?: boolean }) {
  const soldOut = !item.isAvailable;
  return <Link href={`/menu/${item.slug}`} aria-label={`${item.name}, ${money(item.price)}${soldOut ? ", sold out" : ""}`} className={`group relative block overflow-hidden rounded-app bg-white shadow-lift transition duration-200 active:scale-[.98] ${compact ? "w-[178px] shrink-0" : "w-full"}`}>
    <div className={`relative overflow-hidden ${compact ? "h-32" : "h-44"}`}>
      ...
      {soldOut ? <span className="absolute left-3 top-3 rounded-full bg-ink px-2.5 py-1 text-[11px] font-black uppercase tracking-wide text-white">Sold out today</span>
        : item.badge && <span className="absolute left-3 top-3 rounded-full bg-cream px-2.5 py-1 text-[11px] font-black uppercase tracking-wide text-ink">{item.badge}</span>}
    </div>
    <div className="p-3.5">
      <h3 className="min-h-10 text-[.95rem] font-extrabold leading-5 text-ink">{item.name}</h3>
      <div className="mt-2 flex items-center justify-between"><span className={`font-black ${soldOut ? "text-stone-500" : "text-ink"}`}>{money(item.price)}</span><span aria-hidden className={`grid h-8 w-8 place-items-center rounded-full text-white ${soldOut ? "bg-stone-300" : "bg-coral"}`}><ArrowRight size={16} strokeWidth={3} /></span></div>
